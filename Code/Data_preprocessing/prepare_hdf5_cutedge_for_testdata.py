...
data_path = file_paths['src_test_data_path']
# Saved path
target_path = file_paths['src_test_hdf5_path']

test_list = file_paths['test_list']
...
def build_h5_dataset(data_path, target_path):
    ...
    for i in range(24, 40):
        ...
        with h5py.File(os.path.join(target_path, 'test_iseg_norm_cutedge_weight_%s.h5' % i), 'w') as f:
            f['data'] = inputs_caffe  # for caffe num channel x d x h x w

        with open(test_list, 'a') as f:
            f.write(os.path.join(target_path, 'test_iseg_norm_cutedge_weight_%s.h5\n' % i))
