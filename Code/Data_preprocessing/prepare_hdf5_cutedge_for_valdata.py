...
data_path = file_paths['src_val_data_path']
# Saved path
target_path = file_paths['src_val_hdf5_path']
# val list
val_list_file = file_paths['val_list']
...
def build_h5_dataset(data_path, target_path):
    ...
    for i in range(10, 23):
        ...
        mask = labels > 0  # np.unique(mask): [False, True]
        ...
        labels_caffe = labels_caffe.transpose(0, 4, 3, 1, 2)    # batch, channel, depth, width, height

        with h5py.File(os.path.join(target_path, 'val_iseg_norm_cutedge_weight_%s.h5' % (i + 1)), 'w') as f:
            f['data'] = inputs_caffe  # for caffe num channel x d x h x w
            f['label'] = labels_caffe

        with open(val_list_file, 'a') as f:
            f.write(os.path.join(target_path, 'val_iseg_norm_cutedge_weight_%s.h5\n' % (i + 1)))
